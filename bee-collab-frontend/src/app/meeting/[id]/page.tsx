...
import {
  ...
  Send, X, Subtitles, MonitorUp, MoreVertical, Info, Shapes, Shield
} from 'lucide-react';
...
export default function Meeting() {
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'peserta' | null>('chat');
  ...
  useEffect(() => {
    ...
    // Fetch meeting details to know if we are the host
  ...
  return (
    <main style={{ height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', backgroundColor: colors.bgApp, overflow: 'hidden', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', position: 'relative', width: '100%', overflow: 'hidden' }}>
        ...
        {/* Video Grid Area */}
        <div style={{ flex: 1, padding: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s ease' }}>
        ...
        {/* Sidebar */}
        {activeTab && (
          <div style={{ width: '380px', background: colors.bgSidebar, color: '#3c4043', display: 'flex', flexDirection: 'column', borderLeft: '1px solid #dadce0', flexShrink: 0, zIndex: 10 }}>
            <div style={{ display: 'flex', padding: '1.5rem', alignItems: 'center', gap: '1rem' }}>
              <div style={{ flex: 1, background: colors.bgDarkNavy, borderRadius: '16px', display: 'flex', padding: '0.25rem' }}>
              ...
              <button onClick={() => setActiveTab(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8f95a3', padding: '0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <X size={24} />
              </button>
            </div>
      ...
      <div style={{ height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 2rem', background: colors.bgBottomBar, color: '#f1f3f4', flexShrink: 0, zIndex: 20 }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          {isHost ? (
            ...
            }} title="End Meeting for All" style={{ padding: '0 1rem', height: '44px', borderRadius: '22px', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#dc2626', color: 'white', marginLeft: '0.5rem', fontWeight: 'bold', gap: '0.5rem' }}>
              <PhoneOff size={20} /> End Meeting
            </button>
          ) : (
            <button onClick={() => router.push('/')} title="Leave Meeting" style={{ width: '64px', height: '44px', borderRadius: '22px', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: colors.red, color: 'white', marginLeft: '0.5rem' }}>
              <PhoneOff size={20} />
            </button>
          )}
        </div>
